...
export default async function BatchReviewPage({ params }: { params: { batchId: string } }) {
  ...
  return (
    <div style={{ maxWidth: '900px' }}>
      ...
      {/* Post button */}
      <PostButton batchId={params.batchId} pendingCount={pending.length} />
              ...
              <tr key={t.id} style={{ borderBottom: '1px solid #F0F0F0', background: i % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                ...
                <td style={td}>
                  <span style={{
                    ...
                    background: t.status === 'pending' ? '#E3F2FD' : t.status === 'duplicate' ? '#F5F5F5' : t.status === 'posted' ? '#E8F5E9' : '#F5F5F5',
                    color: t.status === 'pending' ? '#1565C0' : t.status === 'duplicate' ? '#999' : t.status === 'posted' ? '#2E7D32' : '#999',
                  }}>
                    {t.status}
                  </span>
                </td>
