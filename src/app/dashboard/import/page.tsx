...
const ACCOUNTS = [
  ...
  { id: 'sync-1629', label: 'Synchrony Sams Club ****1629' },
...
export default function ImportPage() {
  ...
  async function handleUpload() {
    if (!file || !accountId) { setError('Please select a file and account.'); return; }
  ...
  return (
    <div style={{ maxWidth: '640px' }}>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: '#666' }}>Upload a PDF bank or credit card statement to stage transactions.</p>
      </div>
      ...
      <div style={{ background: '#fff', borderRadius: '8px', padding: '2rem', border: '1px solid #E0E0E0', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div>
          ...
          {file && <div style={{ fontSize: '0.85rem', color: '#666', marginTop: '0.4rem' }}>{file.name} ({(file.size/1024).toFixed(0)} KB)</div>}
        </div>
        ...
        {status === 'success' && result && (
          <div style={{ background: '#F1F8E9', border: '1px solid #C5E1A5', borderRadius: '6px', padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.85rem', color: '#444', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              ...
              <div>Duplicates: <strong>{result.duplicates}</strong></div>
            </div>
            <div style={{ marginTop: '1rem' }}>
              <a href={'/dashboard/import/' + result.batchId} style={{ background: '#2E4057', color: '#fff', padding: '0.5rem 1rem', borderRadius: '6px', textDecoration: 'none', fontSize: '0.9rem' }}>
                Review transactions
              </a>
            </div>
          </div>
        ...
        <button
          ...
          style={{ background: status === 'uploading' ? '#999' : '#2E4057', color: '#fff', border: 'none', borderRadius: '6px', padding: '0.75rem 1.5rem', fontSize: '1rem', cursor: status === 'uploading' ? 'not-allowed' : 'pointer', fontWeight: 600 }}
        >
